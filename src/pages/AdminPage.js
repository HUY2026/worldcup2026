...
export default function AdminPage() {
  ...
  const [inputs, setInputs] = useState({}) // matchId -> {home, away, penaltyHome, penaltyAway, hasPenalty}
  ...
  async function handleSaveResult(match) {
    ...
    // Với vòng loại trực tiếp: nếu hòa thì bắt buộc nhập penalty
    ...
    try {
      ...
      const { data: preds, error: predErr } = await supabase.from('predictions').select('*').eq('match_id', match.id)
  ...
  // Trận chưa có kết quả lên trên, đã có xuống dưới
              ...
              <div key={match.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    {/* Penalty toggle — chỉ hiện ở vòng knockout khi tỷ số hòa */}
                    {isKnockout && isTie && (
  ...
  async function handleAddKnockoutMatch() {
    const home = prompt('Tên đội 1 (ví dụ: Pháp):')
    if (!home) return
    const away = prompt('Tên đội 2:')
    if (!away) return
    const round = prompt('Vòng đấu (r32/r16/qf/sf/final):')
    if (!round) return
    const dateInput = prompt('Ngày giờ thi đấu (giờ VN, format: YYYY-MM-DD HH:MM):')
    if (!dateInput) return

    try {
      // Convert VN time to UTC
      const vnDate = new Date(dateInput.replace(' ', 'T') + ':00+07:00')
      const { error } = await supabase.from('matches').insert({
        round, home_team: home, away_team: away,
        match_time: vnDate.toISOString(),
  ...
  return (
    <div>
      ...
      {/* Add knockout match button */}
      <div style={{ marginBottom: 16, display: 'flex', gap: 8 }}>
        <button className="btn btn-accent btn-sm" onClick={handleAddKnockoutMatch}>
          ➕ Thêm trận vòng loại trực tiếp
        </button>
      </div>
